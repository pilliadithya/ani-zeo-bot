...
Model:    configured in config/ai_config.py  (meta/llama-3.3-70b-instruct)
...
────────────────────
  ...
  NVIDIA_API_KEY missing  → error_type = "unconfigured"
"""
...
class NvidiaNimProvider(AIProvider):
    ...
    model_name    = PROVIDER_MODELS.get("nvidia_nim", "meta/llama-3.3-70b-instruct")
    ...
    async def generate_response(
        ...
        try:
            ...
            if resp.status_code != 200:
                ...
                logger.error("NvidiaNimProvider [%s]: %s", error_type, error_msg)
                return ProviderResponse(
                    text="", provider=self.provider_name, model=self.model_name,
            ...
            logger.warning("NvidiaNimProvider timeout: %s", error_msg)
            return ProviderResponse(
                text="", provider=self.provider_name, model=self.model_name,
            ...
            logger.error("NvidiaNimProvider error: %s", error_msg[:200])
            return ProviderResponse(
                text="", provider=self.provider_name, model=self.model_name,
