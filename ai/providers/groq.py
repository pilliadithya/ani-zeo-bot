"""
Groq provider — Llama / Mixtral via Groq's OpenAI-compatible REST API.

Endpoint: https://api.groq.com/openai/v1/chat/completions
SDK:      httpx (already a project dependency — no extra install needed)
Key:      GROQ_API_KEY environment variable
Model:    configured in config/ai_config.py  (llama3-8b-8192 by default)
...
────────────────────
  ...
  GROQ_API_KEY missing    → error_type = "unconfigured"
"""
...
class GroqProvider(AIProvider):
    """
    Groq provider using httpx against the OpenAI-compatible Groq API.

    One httpx.AsyncClient is created per request (Groq recommends short-lived
    clients for serverless-style workloads; the router handles connection reuse
    at a higher level via provider instance caching).
    """
    ...
    async def health_check(self) -> bool:
        """Returns True when GROQ_API_KEY is present — no live API call."""
    ...
    async def generate_response(
        ...
        try:
            ...
            if resp.status_code != 200:
                ...
                logger.error("GroqProvider [%s]: %s", error_type, error_msg)
                return ProviderResponse(
                    text="", provider=self.provider_name, model=self.model_name,
            ...
            if not text:
                return ProviderResponse(
                    text="", provider=self.provider_name, model=self.model_name,
                    success=False, error="Empty response from Groq",
            ...
            logger.warning("GroqProvider timeout: %s", error_msg)
            return ProviderResponse(
                text="", provider=self.provider_name, model=self.model_name,
            ...
            logger.error("GroqProvider error: %s", error_msg[:200])
            return ProviderResponse(
                text="", provider=self.provider_name, model=self.model_name,
